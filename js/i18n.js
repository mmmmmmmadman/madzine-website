    ...
    'zh-TW': {
      ...
      apps: {
        visionmod: {
          ...
          features: {
            pipeline: '模組化訊號流，自由排序、可加入多個同種模組',
            modulation: '畫面與音訊驅動的 modulation 系統',
            ai: 'macOS Stable Diffusion 1.5 / Windows Stream Diffusion',
            effects: 'SDF 3D Object、Particle、Chromascope、Flow Warp、Feedback',
            cvmidi: 'CV／MIDI 輸出控制外部設備（14-bit）',
            crossplatform: '跨平台 Rust + wgpu，macOS 與 Windows 同源碼'
          }
        },
        decapyramid: {
          name: 'DECAPyramid',
        ...
        sessionguide: {
          ...
          platform: 'Web',
          launch: '開啟應用程式',
          intro: 'Session Guide 是為樂團即興與現場演出設計的瀏覽器工具。團長在手機上發出指令，所有團員的手機同步顯示，取代排練與演出中不易傳達的手勢與眼神。<br><br>團長建立房間後會得到一組四字元房號，團員輸入房號即可加入。連線透過 WebRTC 點對點建立，不需要伺服器轉送資料。另有本地模式，不連線也能單機當視覺節拍器使用。<br><br>速度以 Tap Tempo 敲擊設定，取最近四次間隔平均計算 BPM，並支援 4/4、3/4、6/8、7/8、5/4 五種拍號。節拍器為純視覺圓點，拍數對應拍號分子，第一拍以強調色標示，另可選擇開啟提示音。<br><br>指令由格子組成，分為可自訂文字的動作格、力度與速度三組；文字格可自行改字、新增與刪除，設定保存於瀏覽器。指令分兩種：即時指令按下後立刻在所有人畫面全螢幕閃示；小節數指令則進入倒數，剩餘小節以超大字顯示，最後一小節轉為紅色警示，歸零時閃示指令內容。<br><br>各裝置的節拍與倒數皆在本地依時間戳推算，不逐拍依賴網路，並以定期校時修正裝置間的時鐘差。介面為深色極簡設計，支援繁體中文、英文、日文，並在演出期間透過 Wake Lock 保持螢幕常亮。',
          features: {
            sync: 'WebRTC 點對點連線，四字元房號加入，無需帳號',
            tap: 'Tap Tempo 設定速度，全員同步',
            meter: '五種拍號：4/4、3/4、6/8、7/8、5/4',
            metronome: '圖像式節拍器，第一拍強調，提示音可選',
            cues: '指令以四個格子組句：經過多久、馬上或逐漸、做什麼、然後做什麼，按 START 送出',
            ramp: '漸變指令：橫跨數小節的漸強、漸弱、漸快、漸慢，全螢幕進度顯示',
            roles: '每人加入時可選可下指令或只跟隨，演出中可交棒',
        ...
        anyani: {
          name: 'AnyAni',
          desc: '定格動畫製作工具，影片匯入、AI 超解析與補幀',
          platform: 'macOS, Windows',
          intro: 'AnyAni 是一款定格動畫製作工具。可將圖片序列轉換為影片，也可匯入影片抽幀轉為定格動畫。支援逐幀縮放、位移、旋轉與色鍵去背，內建 Fade/Dissolve 轉場效果。<br><br>影片匯入功能支援 mp4、mov、avi、mkv、webm 等格式，依設定的 FPS 自動抽幀，可搭配播放速度控制（含反向播放）製作不同風格的定格動畫。內建 SNS 平台解析度預設（4K、YouTube、Reels/Shorts/TikTok、Instagram Feed 等），支援直式/橫式切換。<br><br>AI 超解析功能使用 Real-ESRGAN 模型，macOS 版透過 CoreML 驅動 Apple Neural Engine 加速（比傳統 Vulkan 快 2-6 倍），採用 512x512 分塊處理搭配重疊混合，保持原始畫面比例不變形。Windows 版使用 ncnn-vulkan，支援 NVIDIA、AMD、Intel 顯示卡。<br><br>RIFE AI 補幀可將影格數提升 2-16 倍，與轉場效果可同時運作。硬體影片編碼優先使用 VideoToolbox（macOS）或 NVENC/AMF/QSV（Windows），軟體回退使用 OpenH264（BSD 授權，無 GPL 依賴）。<br><br>介面使用 Rust + egui 開發，GPU 合成透過 wgpu compute shader 加速。支援英文、日文、繁體中文三語介面。',
          features: {
            videoimport: '影片匯入 → 定格動畫（支援 mp4/mov/avi/mkv/webm 等）',
            upscale: 'AI 超解析（Real-ESRGAN，2x/4x，Photo/Anime 模型）',
            rife: 'AI 補幀（RIFE，2x/4x/8x/16x）',
            speed: '播放速度控制（正向 / 反向）',
            resolution: 'SNS 解析度預設（4K / YouTube / Reels / TikTok / Instagram）',
        ...
        artisttalktranslator: {
          ...
          features: {
            ...
            ui: '中文、英文、日文三語介面',
            stability: '長時間穩定運行：自動重連、外接麥克風熱插拔、連續說話也能翻譯、帳號餘額不足即時提示、應用內 Log 下載',
    ...
    'en': {
      ...
      apps: {
        visionmod: {
          ...
          features: {
            pipeline: 'Modular signal flow with free reordering and multiple instances per module',
            modulation: 'Modulation system driven by image and audio analysis',
            ai: 'Stable Diffusion 1.5 on macOS / Stream Diffusion on Windows',
            effects: 'SDF 3D Object, Particle, Chromascope, Flow Warp, Feedback',
            cvmidi: 'CV / MIDI output for external hardware (14-bit)',
            crossplatform: 'Cross-platform Rust + wgpu, shared codebase on macOS and Windows'
          }
        },
        decapyramid: {
          name: 'DECAPyramid',
        ...
        sessionguide: {
          ...
          features: {
            ...
            cues: 'Cues are composed from four pads — when, instantly or gradually, what, then what — and sent with START',
            ramp: 'Gradual cues: crescendo, diminuendo, accelerando and ritardando across bars, shown full-screen with progress',
            roles: 'Each member joins as conductor or follower, and can hand over mid-performance',
        ...
        anyani: {
          name: 'AnyAni',
          desc: 'Stop motion animation maker with video import, AI upscale and interpolation',
          platform: 'macOS, Windows',
          intro: 'AnyAni is a stop motion animation maker. Convert image sequences into video, or import video and extract frames to create stop-motion animation. Supports per-frame scale, offset, rotation, and chroma key background removal, with built-in Fade/Dissolve transitions.<br><br>Video import supports mp4, mov, avi, mkv, webm and more. Frames are extracted at the configured FPS, with playback speed control (including reverse) for different stop-motion styles. Built-in SNS resolution presets (4K, YouTube, Reels/Shorts/TikTok, Instagram Feed, etc.) with portrait/landscape toggle.<br><br>AI upscaling uses Real-ESRGAN models. On macOS, CoreML drives the Apple Neural Engine for 2-6x faster processing compared to traditional Vulkan, using 512x512 tiling with overlap blending to preserve original aspect ratios. On Windows, ncnn-vulkan supports NVIDIA, AMD, and Intel GPUs.<br><br>RIFE AI interpolation boosts frame count by 2-16x and works simultaneously with transitions. Hardware video encoding prioritizes VideoToolbox (macOS) or NVENC/AMF/QSV (Windows), with OpenH264 (BSD licensed, no GPL dependencies) as software fallback.<br><br>Built with Rust + egui, GPU compositing via wgpu compute shader. Interface supports English, Japanese, and Traditional Chinese.',
          features: {
            videoimport: 'Video Import → Stop Motion (mp4/mov/avi/mkv/webm and more)',
            upscale: 'AI Upscale (Real-ESRGAN, 2x/4x, Photo/Anime models)',
            rife: 'AI Interpolation (RIFE, 2x/4x/8x/16x)',
            speed: 'Playback Speed Control (forward / reverse)',
            resolution: 'SNS Resolution Presets (4K / YouTube / Reels / TikTok / Instagram)',
        ...
        mado: {
          ...
          platform: 'macOS, Windows',
          download: 'Download — macOS / Windows',
        ...
        artisttalktranslator: {
          ...
          features: {
            ...
            stability: 'Long-session stable: auto-reconnect, mic hot-swap, continuous-speech commit, instant account-balance alert, in-app log download',
    ...
    'ja': {
      ...
      apps: {
        visionmod: {
          ...
          features: {
            pipeline: 'モジュール式シグナルフロー、並べ替え自由、同種モジュールの複数 instance 化',
            modulation: '映像と音声の解析で駆動する modulation システム',
            ai: 'macOS Stable Diffusion 1.5 / Windows Stream Diffusion',
            effects: 'SDF 3D Object、Particle、Chromascope、Flow Warp、Feedback',
            cvmidi: 'CV／MIDI 出力で外部機器を制御（14bit）',
            crossplatform: 'クロスプラットフォーム Rust + wgpu、macOS と Windows で同一ソース'
          }
        },
        decapyramid: {
          name: 'DECAPyramid',
        ...
        sessionguide: {
          ...
          features: {
            ...
            cues: 'キューは4つのパッドで組み立て：どれくらい後・すぐに/徐々に・何を・その後に何を、START で送信',
            ramp: '漸次変化キュー：クレッシェンド、ディミヌエンド、アッチェレランド、リタルダンドを全画面で進行表示',
            roles: '参加時にキュー送信可か追従のみかを選択、演奏中の交代も可能',
        ...
        anyani: {
          name: 'AnyAni',
          desc: 'ストップモーション制作ツール、動画取り込み・AI超解像・補間',
          platform: 'macOS, Windows',
          intro: 'AnyAni はストップモーションアニメーション制作ツールです。画像シーケンスを動画に変換するだけでなく、動画を取り込んでフレーム抽出し、ストップモーションアニメーションに変換することもできます。フレームごとのスケール、オフセット、回転、クロマキー背景除去に対応し、Fade/Dissolve トランジション効果を内蔵しています。<br><br>動画取り込みは mp4、mov、avi、mkv、webm などに対応。設定した FPS でフレームを自動抽出し、再生速度コントロール（逆再生を含む）でさまざまなストップモーションスタイルを実現します。SNS 解像度プリセット（4K、YouTube、Reels/Shorts/TikTok、Instagram Feed など）を内蔵し、縦横切り替えにも対応。<br><br>AI 超解像機能は Real-ESRGAN モデルを使用。macOS 版では CoreML が Apple Neural Engine を駆動し、従来の Vulkan と比べ 2-6 倍高速に処理します。512x512 タイル分割とオーバーラップブレンディングにより、元のアスペクト比を維持します。Windows 版は ncnn-vulkan を使用し、NVIDIA、AMD、Intel GPU に対応。<br><br>RIFE AI 補間でフレーム数を 2-16 倍に増加でき、トランジション効果と同時に動作します。ハードウェアエンコードは VideoToolbox（macOS）または NVENC/AMF/QSV（Windows）を優先使用し、ソフトウェアフォールバックには OpenH264（BSD ライセンス、GPL 依存なし）を使用します。<br><br>Rust + egui で開発、wgpu compute shader による GPU 合成。英語、日本語、繁体字中国語の 3 言語インターフェースに対応。',
          features: {
            videoimport: '動画取り込み → ストップモーション（mp4/mov/avi/mkv/webm 等）',
            upscale: 'AI 超解像（Real-ESRGAN、2x/4x、Photo/Anime モデル）',
            rife: 'AI フレーム補間（RIFE、2x/4x/8x/16x）',
            speed: '再生速度コントロール（順方向 / 逆方向）',
            resolution: 'SNS 解像度プリセット（4K / YouTube / Reels / TikTok / Instagram）',
        ...
        artisttalktranslator: {
          ...
          features: {
            ...
            ui: '3言語UI（英語・中国語・日本語）',
            stability: '長時間安定動作：自動再接続、マイク切替対応、連続発話対応、アカウント残高不足の即時通知、アプリ内ログダウンロード',
